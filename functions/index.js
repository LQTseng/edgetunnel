...
export async function onRequest({request,env}) {
	 ...
	 * @param {{UUID: string, PROXYIP: string, PROXYPORT: string, PROXY: object}} env
	 * @param {import("@cloudflare/workers-types").ExecutionContext} ctx
	 * @returns {Promise<Response>}
	 */
	try {
		userID = await env.PROXY.get("uuid") || env.UUID || userID;
		if (!isValidUUID(userID)) throw new Error('uuid is not valid');
		proxyIP = await env.PROXY.get("ip") || env.PROXYIP || proxyIP;
		proxyPort = await env.PROXY.get("port") || env.PROXYPORT || proxyPort;
		socks5Address = await env.PROXY.get("socks5") || env.SOCKS5 || socks5Address;
		if (socks5Address) {
			try {
